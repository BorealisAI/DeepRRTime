...
class ForecastExperiment(Experiment):
    ...
    def instance(self,
        ...
        model.mask_eval = 0.5
        test_metrics_50 = validate(model, loader=test_loader, report_metrics=True,
                                save_path=self.root if save_vals else None)
        results = {'val': val_metrics, 'test': test_metrics, 'test_mask50':test_metrics_50}
        np.save(join(self.root, 'metrics.npy'), results)
...
def train(model: nn.Module,
    ...
    for epoch in range(epochs):
        ...
        for it, data in enumerate(train_loader):
            ...
            forecast, aux_loss = model(x, x_time, y_time)
...
def validate(model: nn.Module,
    ...
    for it, data in enumerate(loader):
        ...
        forecast, _ = model(x, x_time, y_time)
